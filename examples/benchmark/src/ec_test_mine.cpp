...
int main(void)
{
	...
	// Read input
	while(entry_read());
	printf("Encoding 1024 x %u symbols.\n",nentries);
	
	// Encode + decode 1024 times
	std::stringstream buffer;
	CALLGRIND_START_INSTRUMENTATION;
	for(int loop = 0; loop < 10/*24*/; ++loop) {
		...
		for(ei=0;ei<nentries;ei++){
			...
			std::uint64_t low = l>0?f[l-1]:0;
			std::uint64_t high = f[l];
			
			// Rescale from [0, 0x8000] to [0, 2⁶⁴-1]
			low <<= 64 - 15;
			high <<= 64 - 15;
			high -= 1;
		...
		for(ei=0;ei<nentries;ei++){
			...
			// Read current value 
			const std::uint64_t v = er.value();
			const std::uint64_t a = v >> (64 - 15);
			
			// Find it
			for(l = 0; f[l] <= a; ++l);
			
			// Check it
			bool correct = l == entries[ei].l;
			if(!correct) {
